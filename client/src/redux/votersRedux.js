...
export const LOAD_VOTERS = createActionName('LOAD_VOTERS');
export const LIKE_PHOTO = createActionName('LIKE_PHOTO');
...
export const loadVoters = payload => ({ payload, type: LOAD_VOTERS });
export const likePhoto = payload => ({ payload, type: LIKE_PHOTO });
...
export const loadVotersRequest = () => {
  return async dispatch => {

    dispatch(startRequest({ name: LOAD_VOTERS }));
    try {

      let res = await axios.get(`${API_URL}/voters`);

      dispatch(loadVoters(res.data));
      dispatch(endRequest({ name: LOAD_VOTERS }));

    } catch(e) {
      dispatch(errorRequest({ name: LOAD_VOTERS, error: e.message }));
    }

  };
};

export const likePhotoRequest = (id) => {
  return async dispatch => {
